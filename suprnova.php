...
class suprnova {
    ...
    protected function query ($coin, $query) {
        $URL = "http://".$coin.".suprnova.cc/index.php?page=api".
    ...
    public function getBalances() {
        ...
        foreach (array_keys($this->urls) as $coin) {
            ...
            if ($total == 0) continue;
            $balance = array ();
            $balance["name"] = "Suprnova - ".$this->urls[$coin];
    ...
    public function updateMiningData ($sql, $now) {
        ...
        $res = $sql->query ("SELECT * FROM suprnova_transactions");
        if (!$res) {
            /* Create our transaction table */
            $qry = "CREATE TABLE suprnova_transactions ".
                "(id INT NOT NULL,".
...
//            case "Debit_AE":
    ...
    public static function show_setup() {
        printf ("Please fill out the following information to setup your Suprnova.cc account configuration. ");
        printf ("<br><br>\r\n");
        printf ("<table>\r\n");
        printf ("<tr>\r\n");
        printf ("<td>Name:</td>\r\n");
        printf ("<td><input type=\"text\" name=\"account_name\" value=\"Suprnova\"></td>\r\n");
        printf ("</tr>\r\n");
        printf ("<tr>\r\n"); 
    ...
    public static function get_url ($name) {
        if (strstr($name, "Suprnova")) {
            return ("https://dash.suprnova.cc/");
        }
        return ("");
    }
}
